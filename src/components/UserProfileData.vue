        ...
        <v-card-actions>
            <v-spacer></v-spacer>
                ...
                <template v-slot:activator="{ on }">
                    ...
                    <v-btn flat color="orange" v-on="on">Изменить мои данные</v-btn>
                </template>
                <v-card>
                    ...
                    <v-card-text>
                        ...
                        <v-form v-model="valid">
                            ...
                            <v-text-field prepend-icon="lock" name="password" label="Пароль" type="password" required v-model="password" :rules="passwordRules"></v-text-field>
                        <h3>Я хочу изменить</h3>
                           <v-radio-group v-model="changeType">
                            <v-radio label="Имя" :value="'name'"></v-radio>
                                <v-text-field v-if="changeType == 'name'" prepend-icon="person" name="name" label="Новое имя" type="text" required v-model="newname" ></v-text-field>
                            <v-radio label="Эл. адрес" :value="'email'"></v-radio>
                                <v-text-field v-if="changeType == 'email'" prepend-icon="email" name="login" label="Новая почта" type="email" required v-model="newemail" ></v-text-field>
                            <v-radio label="Пароль" :value="'password'"></v-radio>
                                <v-text-field v-if="changeType == 'password'" prepend-icon="lock" name="password" label="Новый пароль" type="password" required v-model="newpassword" ></v-text-field>
                        </v-radio-group>
                        </v-form>
                    </v-card-text>
                    <v-card-actions>
